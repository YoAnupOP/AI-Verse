  ...
  return (
    <Link href={`/chatrooms/${id}`} className="flex-shrink-0 w-[230px] md:w-full block" aria-label={`View details for chatroom: ${name}`}>
      ...
      >
        ...
        <div className="flex-1 p-2.5 sm:p-3.5 md:p-4 flex flex-col relative z-[2]">
          ...
          {/* Description - ADDED min-height for consistency */}
          {/* text-xs has line-height: 1rem (h-4). 2 lines = h-8 (2rem) */}
          {/* sm:text-sm has line-height: 1.25rem (h-5). 2 lines = h-10 (2.5rem) */}
          <p className="text-gray-200 text-xs sm:text-sm mb-2 sm:mb-3 line-clamp-2 min-h-[2rem] sm:min-h-[2.5rem]"> {/* Tailwind: min-h-8 sm:min-h-10 */}
            {description}
          </p>
