...
export default function ChatroomCard({ 
  ...
  const handleGoButtonClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    if (portalCardRef.current) {
      portalCardRef.current.classList.add('portal-card-zoom');
      portalCardRef.current.classList.add('zoomed-in');
      setTimeout(() => {
        if (portalCardRef.current) {
          portalCardRef.current.classList.remove('zoomed-in');
          // Optionally remove portal-card-zoom if it's only for the animation duration
          // portalCardRef.current.classList.remove('portal-card-zoom');
        }
      }, 2000); // The portal-card-zoom transition is 2s for transform
    }
  };
