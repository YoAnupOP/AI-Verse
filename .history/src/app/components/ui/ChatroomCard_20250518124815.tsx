...
export default function ChatroomCard({ 
  ...
  const handleGoButtonClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    // Apply pulse effect to the button itself
    if (goButtonRef.current) {
      goButtonRef.current.classList.add('pulse-effect');
      setTimeout(() => {
        if (goButtonRef.current) {
          goButtonRef.current.classList.remove('pulse-effect');
        }
      }, 600); // Duration of the pulse-effect animation
    }

    // If you still want some effect on the main card (optional, remove if not needed)
    // if (portalCardRef.current) {
    //   portalCardRef.current.classList.add('some-other-card-effect'); // Example
    //   setTimeout(() => {
    //     if (portalCardRef.current) {
    //       portalCardRef.current.classList.remove('some-other-card-effect');
    //     }
    //   }, 600);
    // }
  };
  ...
  return (
    <Link href={`/chatrooms/${id}`} className="flex-shrink-0 w-[230px] md:w-full block" aria-label={`View details for chatroom: ${name}`}>
      <div
        ...
        className="
          portal-card-pseudos
          w-full h-[290px] sm:h-[340px] md:h-[360px]
      ...
      >
        ...
        <div className="relative w-full h-[155px] sm:h-[165px] md:h-[175px] flex-shrink-0">
          <img 
            src={thumbnailError ? fallbackThumbnail : thumbnail}
            alt={`Thumbnail for ${name}`}
            className="w-full h-full" // Assuming you want object-fill or object-cover here
        ...
        <div className="flex-1 p-2 sm:p-3 md:p-4 flex flex-col z-[2]"> {/* REMOVED relative bottom-2. Adjusted padding for consistency. */}
          {/* Chatroom Name & Status */}
          <div className="mb-1 sm:mb-1.5"> {/* Slightly reduced bottom margin */}
            <h2 className="text-white font-bold text-lg sm:text-xl md:text-2xl tracking-wide leading-tight truncate">
              {name}
            </h2>
            <div className="flex items-center mt-0.5"> {/* Added small top margin for status */}
          ...
          {/* Description */}
          <p className="text-gray-200 text-xs sm:text-sm line-clamp-2 mb-1 sm:mb-1.5"> {/* Added bottom margin */}
            {description}
          </p>

          {/* Tags */}
          <div className="flex flex-wrap gap-1 sm:gap-1.5"> {/* Adjusted gap and bottom margin */}
            {tags.slice(0, 3).map((tag, index) => (
              <span key={index} className="bg-[rgba(0,255,170,0.15)] text-[#00ffaa] text-xs px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full"> {/* Adjusted padding */}
                {tag}
              </span>
            ))}
            {tags.length > 3 && (
              <span className="bg-[rgba(100,100,100,0.2)] text-gray-300 text-xs px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full"> {/* Adjusted padding */}
                +{tags.length - 3} more
              </span>
            )}
          </div>
          ...
          <div className="mt-auto flex items-center justify-between pt-1"> {/* ADDED mt-auto and small top padding to separate from tags */}
            <div className="flex items-center">
              {/* Avatars */}
              <div className="flex -space-x-1 sm:-space-x-2 mr-1.5 sm:mr-2"> {/* Adjusted margin */}
                {[1, 2, 3].map(i => (
                  <div key={i} className="w-6 h-6 sm:w-7 sm:h-7 rounded-full overflow-hidden border-2 border-gray-900 flex-shrink-0"> {/* Removed md avatar size for consistency with mobile-first */}
                    <img 
                ...
                {(Number(activeUsers) > 3) && (
                  <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-gray-800 border-2 border-gray-900 flex items-center justify-center text-xs text-white flex-shrink-0"> {/* Removed md avatar size */}
                    +{Number(activeUsers) - 3}
                  </div>
                )}
              </div>
              ...
              {Number(activeUsers) > 0 && (
                <div className="text-white text-xs sm:text-sm">
                  <span className="mr-0.5 sm:mr-1" role="img" aria-label="users">👤</span> {activeUsers} {/* Adjusted margin */}
                </div>
              )}
            </div>
            ...
            <button 
              type="button"
              className="portal-button-pseudo py-1 sm:py-1.5 px-3 sm:px-4 md:px-6 text-sm sm:text-base md:text-lg bg-[rgba(10,12,20,0.3)] border-2 border-[#00ffaa] rounded-full text-white font-bold tracking-[1px] cursor-pointer transition-all duration-300 ease-in-out relative z-10 overflow-hidden shadow-portal-button text-shadow-portal-button hover:scale-105 hover:shadow-portal-button-hover hover:text-shadow-portal-button-hover hover:border-[#00ffdd] group-hover:border-[#00ffee]" // Adjusted padding and text sizes
